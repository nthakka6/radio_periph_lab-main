...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    float tt_sec = (float)total_time/(125*1000000.0);
    ...
    unsigned int bytes_transferred = 4*2048; //4*total_time; // change obviously
    //float time_spent = 1; // change obviously
    throughput = bytes_transferred/tt_sec;
    printf("You transferred %f bytes of data in %f seconds\n",(float)bytes_transferred,(float)tt_sec);
    printf("Measured Transfer throughput = %f Mbytes/sec\n",(float)throughput/1000000.0);
}
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Niren Thakkar - Custom Peripheral Demonstration\n\r");
